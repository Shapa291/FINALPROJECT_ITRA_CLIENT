...
const RateProblem = () => {
    ...
    const rateProblem = (id, value) => {
        PostProblemRate(id, value).then(() => {
            GetOneProblem(id).then((resp) => {
                setOneProblemState(resp)
                let rateData = []
                for (let i = 0; i < resp.Rates.length; i++) {
                    rateData.push(resp.Rates[i].rate);
                }
                let averageRate = (rateData.reduce((a, b) => (a + b)) / rateData.length).toFixed(1)
                PutProblemRate(id, averageRate).then(() => {
